CREATE OR REPLACE PROCEDURE ModelSelection(target_table TEXT)
LANGUAGE plpython3u AS $$
    ...
    Target_table should have two columns: text and label.
    """
    import time
    import json
    import pandas as pd
    ...
    query = "SELECT * FROM PERF_MATRIX"
    PERF_MATRIX = plpy.execute(query)
    query = "SELECT * FROM CONV_TREND"
    CONV_MATRIX = plpy.execute(query)
    query = "SELECT * FROM CONV_TREND_FINAL"
